...
Checklist for integrating a new messaging platform into the Morpheus gateway.
...
## 12. Status Display (`morpheus_cli/status.py`)
...
## 13. Gateway Setup Wizard (`morpheus_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ morpheus_cli/ toolsets.py \
  --include="*.py" -l | sort -u
