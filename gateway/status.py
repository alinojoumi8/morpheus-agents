...
The PID file lives at ``{MORPHEUS_HOME}/gateway.pid``.  MORPHEUS_HOME defaults to
``~/.morpheus`` but can be overridden via the environment variable.  This means
separate MORPHEUS_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting MORPHEUS_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "morpheus" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Morpheus gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different MORPHEUS_HOME dirs).
    """
