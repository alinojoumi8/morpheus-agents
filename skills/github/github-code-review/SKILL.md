...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Morpheus Agent\",
    \"comments\": [
...
```bash
source ~/.morpheus/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Morpheus Agent. Code looks clean — good test coverage, no security concerns."
