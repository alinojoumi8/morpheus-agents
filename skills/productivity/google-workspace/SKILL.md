...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Morpheus venv.
version: 1.0.0
...
```bash
GSETUP="python ~/.morpheus/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `~/.morpheus/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.morpheus/google_oauth_pending.json` until exchange completes.
...
```bash
GAPI="python ~/.morpheus/skills/productivity/google-workspace/scripts/google_api.py"
```
