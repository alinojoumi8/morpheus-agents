...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.morpheus/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MORPHEUS_HOME", Path.home() / ".morpheus")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Morpheus config paths
# ═══════════════════════════════════════════════════════════════════

MORPHEUS_HOME = Path(os.getenv("MORPHEUS_HOME", Path.home() / ".morpheus"))
CONFIG_PATH = MORPHEUS_HOME / "config.yaml"
PREFILL_PATH = MORPHEUS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Morpheus models — already uncensored, just needs clean prompt
    "morpheus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "morpheus" in model_lower or "nous" in model_lower:
        return "morpheus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Morpheus config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.morpheus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Morpheus for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Morpheus for changes to take effect.")
