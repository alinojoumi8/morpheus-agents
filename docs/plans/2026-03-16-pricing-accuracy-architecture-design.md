...
Morpheus should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current Morpheus behavior has four structural issues:
...
Morpheus must stop keying pricing solely by `model`.
...
Live requests may produce only an estimate initially. Morpheus should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. Morpheus stores canonical usage plus reconciliation ids.
3. Morpheus computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. Morpheus should instead migrate to:
...
This is the minimum architecture that makes the statement "Morpheus pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
