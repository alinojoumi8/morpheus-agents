"""ACP permission bridging — maps ACP approval requests to morpheus approval callbacks."""
...
# Maps ACP PermissionOptionKind -> morpheus approval result strings
_KIND_TO_MORPHEUS = {
    "allow_once": "once",
    "allow_always": "always",
    "reject_once": "deny",
...
def make_approval_callback(
    ...
    """
    Return a morpheus-compatible ``approval_callback(command, description) -> str``
        ...
        if isinstance(outcome, AllowedOutcome):
            ...
            for opt in options:
                if opt.option_id == option_id:
                    return _KIND_TO_MORPHEUS.get(opt.kind, "deny")
