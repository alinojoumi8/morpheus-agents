"""ACP agent server — exposes Morpheus Agent via the Agent Client Protocol."""
...
try:
    from morpheus_cli import __version__ as MORPHEUS_VERSION
except Exception:
    MORPHEUS_VERSION = "0.0.0"
...
class MorpheusACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Morpheus AIAgent."""
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethod(
                    ...
                    description=f"Authenticate Morpheus using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="morpheus-agent", version=MORPHEUS_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Morpheus on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from morpheus_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            ...
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["morpheus-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Morpheus Agent v{MORPHEUS_VERSION}"
