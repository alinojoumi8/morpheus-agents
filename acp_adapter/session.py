"""ACP session manager — maps ACP sessions to Morpheus AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.morpheus/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed morpheus agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Morpheus AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Morpheus runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.morpheus/state.db``) is lazily created.
        """
