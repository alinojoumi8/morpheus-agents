...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Morpheus persistent cross-session memory.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Morpheus asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Morpheus + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Morpheus\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Morpheus persistent memory")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'morpheus'")
        print("  with Honcho configured and no prior session history.")
        print("  (Morpheus calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Morpheus, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Morpheus:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Morpheus:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Morpheus:   Honcho observes every message and updates representations")
