...
version: 1.0.0
author: arceus77-7, enhanced by Morpheus Agent
license: MIT
metadata:
  morpheus:
...
- `tmux` available for stable authenticated sessions during Morpheus terminal calls (desktop app flow only)
...
### Service Account (recommended for Morpheus)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.morpheus/.env` (the skill will prompt for this on first load).
...
## Morpheus Execution Pattern (desktop app flow)

Morpheus terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/morpheus-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/morpheus-op.sock"
