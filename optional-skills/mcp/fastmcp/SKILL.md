...
- smoke-test a server with the FastMCP CLI before wiring it into Morpheus or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Morpheus. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.morpheus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.morpheus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Morpheus integration, either:

- configure the server in `~/.morpheus/config.yaml` using the `native-mcp` skill, or
...
### Morpheus cannot see the deployed server

The server-building part may be correct while the Morpheus config is not. Load the `native-mcp` skill and configure the server in `~/.morpheus/config.yaml`, then restart Morpheus.
