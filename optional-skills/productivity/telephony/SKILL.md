---
name: telephony
description: Give Morpheus phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
This optional skill gives Morpheus practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.morpheus/.env`
...
It does **not** turn Morpheus into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Morpheus memory
...
### 1) "I want Morpheus to own a real phone number"
Use **Twilio**.
...
- pairs well with Morpheus `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.morpheus/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
Then save credentials into Morpheus:
...
3. Buy it and save it into `~/.morpheus/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Morpheus calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Morpheus's existing `text_to_speech` support.

Use this when:
- you want the call to use Morpheus's configured TTS voice rather than Twilio `<Say>`
...
Recommended Morpheus TTS -> Twilio Play workflow:

1. Generate the audio with Morpheus `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Morpheus to create the message audio with `text_to_speech`
...
Important note:
- Morpheus TTS is great for prerecorded outbound messages
...
- Morpheus STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Morpheus memory.
...
- Do not store arbitrary third-party phone numbers in Morpheus memory.
...
3. persist that number to `~/.morpheus/.env`
