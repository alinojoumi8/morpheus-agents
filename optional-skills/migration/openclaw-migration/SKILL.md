...
description: Migrate a user's OpenClaw customization footprint into Morpheus Agent. Imports Morpheus-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
...
# OpenClaw -> Morpheus Migration

Use this skill when a user wants to move their OpenClaw setup into Morpheus Agent with minimal manual cleanup.
...
- merge OpenClaw command approval patterns into Morpheus `command_allowlist`
- migrate Morpheus-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.morpheus/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Morpheus workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.morpheus/tts/`
- archive non-secret docs that do not have a direct Morpheus destination
...
1. Prefer the installed path under `~/.morpheus/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Morpheus-compatible secrets, currently:
...
Morpheus CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Morpheus. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Morpheus workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Morpheus skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Morpheus unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Morpheus targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Morpheus destination. Unsupported auth blobs must still be reported as skipped.
...
- Morpheus persona state imported
- Morpheus memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.morpheus/skills/openclaw-imports/`
