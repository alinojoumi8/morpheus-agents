"""Tests for morpheus-api-server toolset and API server tool availability."""
import os
import json
...
class TestMorpheusApiServerToolset:
    """Tests for the morpheus-api-server toolset definition."""

    def test_toolset_exists(self):
        ts = get_toolset("morpheus-api-server")
        assert ts is not None

    def test_toolset_validates(self):
        assert validate_toolset("morpheus-api-server")

    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("morpheus-api-server")
...
class TestApiServerPlatformConfig:
    def test_platforms_dict_includes_api_server(self):
        from morpheus_cli.tools_config import PLATFORMS
        assert "api_server" in PLATFORMS
        assert PLATFORMS["api_server"]["default_toolset"] == "morpheus-api-server"
...
class TestApiServerAdapterToolset:
    ...
    def test_create_agent_reads_config_toolsets(self):
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
            ...
            # No platform_toolsets override — should fall back to morpheus-api-server default
