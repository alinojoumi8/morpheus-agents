"""Tests for morpheus_cli configuration management."""
...
class TestGetMorpheusHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MORPHEUS_HOME", None)
            home = get_morpheus_home()
            assert home == Path.home() / ".morpheus"

    def test_env_override(self):
        with patch.dict(os.environ, {"MORPHEUS_HOME": "/custom/path"}):
            home = get_morpheus_home()
            assert home == Path("/custom/path")


class TestEnsureMorpheusHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"MORPHEUS_HOME": str(tmp_path)}):
            ensure_morpheus_home()
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"MORPHEUS_HOME": str(tmp_path)}, clear=False):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from morpheus_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
