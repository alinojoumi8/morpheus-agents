...
class TestLocalOneShotRegression:
    ...
    def test_state_does_not_persist(self):
        env = LocalEnvironment(persistent=False)
        env.execute("export MORPHEUS_ONESHOT_LOCAL=yes")
        r = env.execute("echo $MORPHEUS_ONESHOT_LOCAL")
...
class TestLocalPersistent:
    ...
    def test_env_var_persists(self, env):
        env.execute("export MORPHEUS_LOCAL_PERSIST_TEST=works")
        r = env.execute("echo $MORPHEUS_LOCAL_PERSIST_TEST")
