...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._morpheus_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_morpheus_bin(self):
        """After _INSTALL_FAILED, manual install in MORPHEUS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        morpheus_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(morpheus_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(morpheus_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._morpheus_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == morpheus_bin
            assert _tirith_mod._resolved_path == morpheus_bin
            mock_clear.assert_called_once()
...
class TestMorpheusHomeIsolation:
    def test_morpheus_bin_dir_respects_morpheus_home(self):
        """_morpheus_bin_dir must use MORPHEUS_HOME, not hardcoded ~/.morpheus."""
        from tools.tirith_security import _morpheus_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"MORPHEUS_HOME": tmpdir}):
            result = _morpheus_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_morpheus_home(self):
        """_failure_marker_path must use MORPHEUS_HOME, not hardcoded ~/.morpheus."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"MORPHEUS_HOME": "/custom/morpheus"}):
            result = _failure_marker_path()
        assert result == "/custom/morpheus/.tirith-install-failed"
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        morpheus_home = os.getenv("MORPHEUS_HOME")
        assert morpheus_home is not None, "MORPHEUS_HOME should be set by conftest"
        assert "morpheus_test" in morpheus_home, "Should point to test temp dir"
