...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("morpheus_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("morpheus_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("morpheus_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("morpheus_cli.auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "morpheus_cli.auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "morpheus_cli.auth.os.access",
        ...
        monkeypatch.setattr("morpheus_cli.auth.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("morpheus_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "morpheus_cli.runtime_provider._get_model_config",
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from morpheus_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from morpheus_cli.main import _PROVIDER_MODELS as main_models
        from morpheus_cli.models import _PROVIDER_MODELS as models_models
