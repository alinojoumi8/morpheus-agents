...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Morpheus Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Morpheus Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Morpheus Agent",
    },
    {
...
class TerminalTestEnv(MorpheusAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/morpheus-agent/.env.
        """
