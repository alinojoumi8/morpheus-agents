...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs MorpheusAgentLoop with terminal tool only
...
Requires: pip install morpheus-agent[yc-bench]
"""
...
class YCBenchEvalConfig(MorpheusAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends MorpheusAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(MorpheusAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The MorpheusAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs MorpheusAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our MorpheusAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the MorpheusAgentLoop
            # ----------------------------------------------------------
