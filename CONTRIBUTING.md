...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. JSON logs go to `~/.morpheus/sessions/`.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hermes setup` or update `~/.morpheus/.env` locally.
...
- Include: OS, Python version, Morpheus version (`hermes version`), full error traceback
