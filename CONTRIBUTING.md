# Contributing to Morpheus Agent

Thank you for contributing to Morpheus Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Morpheus to work everywhere.
...
Bundled skills (in `skills/`) ship with every Morpheus install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `morpheus skills browse` (labeled "official") and install it with `morpheus skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `morpheus skills install`.
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/morpheus-agent.git
cd morpheus-agent
...
```bash
mkdir -p ~/.morpheus/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.morpheus/config.yaml
touch ~/.morpheus/.env
...
├── cli.py                    # MorpheusCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (morpheus-cli, morpheus-telegram, etc.)
├── morpheus_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── morpheus_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.morpheus/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (morpheus-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.morpheus/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.morpheus/`)

| Path | Purpose |
|------|---------|
| `~/.morpheus/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.morpheus/.env` | API keys and secrets |
| `~/.morpheus/auth.json` | OAuth credentials (Nous Portal) |
| `~/.morpheus/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.morpheus/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.morpheus/state.db` | SQLite session database |
| `~/.morpheus/sessions/` | JSON session logs |
| `~/.morpheus/cron/` | Scheduled job data |
| `~/.morpheus/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`morpheus_state.py`) with full-text search and unique session titles. JSON logs go to `~/.morpheus/sessions/`.
...
Four fields are supported under `metadata.morpheus`:
...
The user may skip setup and keep loading the skill. Morpheus only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `morpheus setup` or update `~/.morpheus/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Morpheus tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `morpheus --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Morpheus uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.morpheus/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `morpheus_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `morpheus_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Morpheus runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Morpheus has terminal access. Security matters.
...
2. **Test manually**: Run `morpheus` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/morpheus-agent/issues)
- Include: OS, Python version, Morpheus version (`morpheus version`), full error traceback
