...
from morpheus_cli.config import get_env_value, get_morpheus_home, save_env_value, is_managed, managed_error
from morpheus_cli.setup import (
...
def get_service_name() -> str:
    ...
    home = get_morpheus_home().resolve()
    default = (_Path.home() / ".morpheus").resolve()
    if home == default:
...
User={username}
Group={group_name}
ExecStart={python_path} -m morpheus_cli.main gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Morpheus install")
    return True
...
def generate_launchd_plist() -> str:
    python_path = get_python_path()
    working_dir = str(PROJECT_ROOT)
    log_dir = get_morpheus_home() / "logs"
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>morpheus_cli.main</string>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Morpheus install")
    return True
...
def run_gateway(verbose: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Morpheus Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Morpheus will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Morpheus delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your morpheus agent",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_morpheus_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_whatsapp():
    ...
    from morpheus_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"MorpheusAgent\"")
