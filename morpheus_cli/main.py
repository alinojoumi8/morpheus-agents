...
from morpheus_cli import __version__, __release_date__
from morpheus_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from morpheus_cli.config import get_env_path, get_morpheus_home
    from morpheus_cli.auth import get_auth_status

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from morpheus_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_morpheus_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def cmd_chat(args):
    ...
    try:
        from morpheus_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from morpheus_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from morpheus_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Morpheus?")
        print()
    ...
    session_dir = get_morpheus_home() / "whatsapp" / "session"
...
def cmd_setup(args):
    ...
    from morpheus_cli.setup import run_setup_wizard
    run_setup_wizard(args)


def cmd_model(args):
    """Select default model — starts with provider selection, then model picker."""
    from morpheus_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from morpheus_cli.config import load_config, get_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from morpheus_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from morpheus_cli.config import get_env_value, save_env_value
    ...
    from morpheus_cli.models import model_ids
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    try:
        from morpheus_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from morpheus_cli.auth import _save_model_choice, deactivate_provider
    from morpheus_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from morpheus_cli.models import probe_api_models
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from morpheus_cli.auth import _save_model_choice, deactivate_provider
    from morpheus_cli.config import save_env_value, load_config, save_config
    from morpheus_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from morpheus_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from morpheus_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Morpheus turns to `copilot --acp`.")
    print("  Morpheus currently starts its own ACP subprocess for each request.")
    print("  Morpheus uses your selected model as a hint for the Copilot ACP session.")
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print("    Morpheus will use Claude's credential store directly instead of copying a setup-token into ~/.morpheus/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from morpheus_cli.models import _PROVIDER_MODELS
...
def cmd_login(args):
    """Authenticate Morpheus CLI with a provider."""
    from morpheus_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from morpheus_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from morpheus_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from morpheus_cli.cron import cron_command
    cron_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from morpheus_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from morpheus_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Morpheus Agent v{__version__} ({__release_date__})")
    ...
    try:
        from morpheus_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    ...
    from morpheus_cli.uninstall import run_uninstall
    run_uninstall(args)


def _update_via_zip(args):
    """Update Morpheus Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/morpheus-agent/archive/refs/heads/{branch}.zip"
    ...
    try:
        ...
        # GitHub ZIPs extract to morpheus-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"morpheus-agent-{branch}")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Morpheus behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Morpheus can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Morpheus couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Morpheus couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Morpheus behaves unexpectedly.")
    return True
...
def cmd_update(args):
    """Update Morpheus Agent to the latest version."""
    import shutil
    
    print("⚕ Updating Morpheus Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/morpheus-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # Uses the PID file (scoped to MORPHEUS_HOME) to find this
        # installation's gateway — safe with multiple installations.
        try:
            from gateway.status import get_running_pid, remove_pid_file
            from morpheus_cli.gateway import (
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Morpheus Agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Morpheus CLI"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Morpheus Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Morpheus Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Morpheus Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from morpheus_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            from morpheus_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from morpheus_cli.skills_hub import skills_command
            skills_command(args)
    ...
    def cmd_plugins(args):
        from morpheus_cli.plugins_cmd import plugins_command
    ...
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "each turn so Morpheus knows who you are without you having to repeat yourself.\n\n"
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Morpheus Honcho",
    )
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            from morpheus_cli.tools_config import tools_command
            tools_command(args)
    ...
    def cmd_mcp(args):
        from morpheus_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            if hermes_bin:
                ...
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "morpheus_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Morpheus"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Morpheus",
    ...
    def cmd_claw(args):
        from morpheus_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Morpheus Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Morpheus Agent",
        description="Remove Morpheus Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Morpheus Agent as an ACP (Agent Client Protocol) server",
        description="Start Morpheus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Morpheus Agent as an ACP server."""
        try:
