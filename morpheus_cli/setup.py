"""
Interactive setup wizard for Morpheus Agent.
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from morpheus_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    """
    from morpheus_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Morpheus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from morpheus_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    print(color("📁 All your files are in ~/.morpheus/:", Colors.CYAN, Colors.BOLD))
    print()
...
def setup_model_provider(config: dict):
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for Morpheus to work.")
        print()
    ...
    if provider_idx == 0:  # OpenRouter
        ...
        try:
            from morpheus_cli.auth import deactivate_provider
        ...
        config_path = (
            Path(os.environ.get("MORPHEUS_HOME", Path.home() / ".morpheus")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        try:
            from morpheus_cli.auth import _login_nous
            import argparse
        ...
        if api_key:
            ...
            from morpheus_cli.auth import detect_zai_endpoint
        ...
        from morpheus_cli.config import save_anthropic_api_key, save_anthropic_oauth_token
        ...
        print_info("Morpheus can use GITHUB_TOKEN, GH_TOKEN, or your gh CLI login.")
        print_info(f"Base URL: {pconfig.inference_base_url}")
        print()
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend Morpheus can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Morpheus can auto-use for vision,")
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from morpheus_cli.models import provider_model_ids
            ...
            from morpheus_cli.models import model_ids, menu_labels
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Morpheus runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Morpheus via Telegram/Discord, this is where")
        print_info(
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Morpheus from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Morpheus delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://morpheus-agent.nousresearch.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where Morpheus delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where Morpheus delivers cron job results and notifications.")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://morpheus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/"
        )
        print()
        ...
        print_info("   1. Define webhook routes in ~/.morpheus/config.yaml")
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from morpheus_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from morpheus_cli.auth import get_active_provider
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Morpheus can import your settings, memories, skills, and API keys.")
    print()
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Morpheus.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Morpheus Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Morpheus Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Morpheus Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Morpheus configured.")
        print()
