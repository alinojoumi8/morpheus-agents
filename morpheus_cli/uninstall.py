"""
Morpheus Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.morpheus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Morpheus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Morpheus Agent" comment and following line
                if '# Morpheus Agent' in line or '# morpheus-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('morpheus' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains morpheus_cli reference)
                content = wrapper.read_text()
                if 'morpheus_cli' in content or 'morpheus-agent' in content:
                    wrapper.unlink()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.morpheus/ (configs, data, logs)
    ...
    print(color("│            ⚕ Morpheus Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Morpheus data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
    else:
        print("This will remove the Morpheus code but keep your configuration and data.")
    ...
    # 5. Optionally remove ~/.morpheus/ data directory
    if full_uninstall:
    ...
    print()
    print("Thank you for using Morpheus Agent! ⚕")
    print()
