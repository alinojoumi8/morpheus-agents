...
Saves per-platform tool configuration to ~/.morpheus/config.yaml under
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from morpheus_cli.plugins import discover_plugins, get_plugin_toolsets
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning("    npm install failed - run manually: cd ~/.morpheus/morpheus-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in morpheus-agent directory)")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Morpheus Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    print(color("  Tool configuration saved to ~/.morpheus/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'morpheus' or gateway restart.", Colors.DIM))
    print()
