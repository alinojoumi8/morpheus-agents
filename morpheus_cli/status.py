...
from morpheus_cli.auth import AuthError, resolve_provider
from morpheus_cli.colors import Colors, color
from morpheus_cli.config import get_env_path, get_env_value, get_morpheus_home, load_config
from morpheus_cli.models import provider_label
from morpheus_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Morpheus Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Morpheus Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    jobs_file = get_morpheus_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_morpheus_home() / "sessions" / "sessions.json"
