...
from morpheus_cli.config import get_project_root, get_morpheus_home, get_env_path

PROJECT_ROOT = get_project_root()
MORPHEUS_HOME = get_morpheus_home()

# Load environment variables from ~/.morpheus/.env so API key checks work
...
from morpheus_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.morpheus/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Morpheus Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.morpheus/.env (primary location for user config)
    env_path = MORPHEUS_HOME / '.env'
    if env_path.exists():
        check_ok("~/.morpheus/.env file exists")
        ...
        if fallback_env.exists():
            ...
            check_fail("~/.morpheus/.env file missing")
            if should_fix:
    ...
    if config_path.exists():
        check_ok("~/.morpheus/config.yaml exists")
    else:
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok("Created ~/.morpheus/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append("Create ~/.morpheus/config.yaml manually")
            else:
    ...
    try:
        from morpheus_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    if hermes_home.exists():
        ...
        if should_fix:
            ...
            check_warn("~/.morpheus not found", "(will be created on first use)")
    ...
    for subdir_name in expected_subdirs:
        ...
        if subdir_path.exists():
            ...
            if should_fix:
                ...
                check_ok(f"Created ~/.morpheus/{subdir_name}/")
                fixed_count += 1
            else:
    ...
    if soul_path.exists():
        ...
        if lines:
            check_ok("~/.morpheus/SOUL.md exists (persona configured)")
        else:
            check_info("~/.morpheus/SOUL.md exists but is empty — edit it to customize personality")
    else:
        check_warn("~/.morpheus/SOUL.md not found", "(create it to give Morpheus a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Morpheus Agent Persona\n\n"
                "<!-- Edit this file to customize how Morpheus communicates. -->\n\n"
                "You are Morpheus, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            check_ok("Created ~/.morpheus/SOUL.md with basic template")
            fixed_count += 1
    ...
    if memories_dir.exists():
        ...
        check_warn("~/.morpheus/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.morpheus/memories/")
            fixed_count += 1
    ...
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"~/.morpheus/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"~/.morpheus/state.db exists but has issues: {e}")
    else:
        check_info("~/.morpheus/state.db not created yet (will be created on first session)")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/morpheus-agent/issues/811
    ...
    hub_dir = MORPHEUS_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    if github_token:
        ...
        check_warn("No GITHUB_TOKEN", "(60 req/hr rate limit — set in ~/.morpheus/.env for better rates)")
