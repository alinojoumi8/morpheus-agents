...
configuration in ~/.morpheus/config.yaml under the ``mcp_servers`` key.
"""
...
from morpheus_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_morpheus_home,  # noqa: F401 — used by test mocks
)
...
def cmd_mcp_add(args):
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to ~/.morpheus/.env as {env_key}")
    ...
    print()
    _success(f"Saved '{name}' to ~/.morpheus/config.yaml ({tool_count}/{total} tools enabled)")
