# OpenAI-Compatible API Server for Morpheus Agent
...
SSE streaming. By exposing this endpoint, morpheus-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  morpheus-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.morpheus/.env`
2. Run `morpheus gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with morpheus-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns morpheus-agent as a model) |
...
Authorization: Bearer morpheus-api-key-here
...
But morpheus-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "morpheus-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured morpheus-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of morpheus-agent's core prompt
...
Once implemented, morpheus-agent works as a drop-in backend for:
