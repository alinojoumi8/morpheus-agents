...
it is automatically downloaded from GitHub releases to $MORPHEUS_HOME/bin/tirith.
...
def _get_morpheus_home() -> str:
    """Return the Morpheus home directory, respecting MORPHEUS_HOME env var."""
    return str(get_morpheus_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_morpheus_home(), ".tirith-install-failed")
...
def _morpheus_bin_dir() -> str:
    """Return $MORPHEUS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_morpheus_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MORPHEUS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_morpheus_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $MORPHEUS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $MORPHEUS_HOME/bin/tirith
    ...
    morpheus_bin = os.path.join(_morpheus_bin_dir(), "tirith")
    if os.path.isfile(morpheus_bin) and os.access(morpheus_bin, os.X_OK):
        _resolved_path = morpheus_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return morpheus_bin
