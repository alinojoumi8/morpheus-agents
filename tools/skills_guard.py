...
Trust levels:
  - builtin:   Ships with Morpheus. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.morpheus/\.env|\~/\.morpheus/\.env',
     "morpheus_env_access", "critical", "exfiltration",
     "directly references Morpheus secrets file"),
    ...
    (r'\.morpheus/config\.yaml|\.morpheus/SOUL\.md',
     "morpheus_config_mod", "critical", "persistence",
     "references Morpheus configuration files directly"),
...
def _get_configured_model() -> str:
    """Load the user's configured model from ~/.morpheus/config.yaml."""
    try:
