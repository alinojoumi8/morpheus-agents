...
DANGEROUS_PATTERNS = [
    ...
    (r'\btee\b.*(/etc/|/dev/sd|\.ssh/|\.morpheus/\.env)', "overwrite system file via tee"),
    (r'\bxargs\s+.*\brm\b', "xargs with rm"),
    (r'\bfind\b.*-exec\s+(/\S*/)?rm\b', "find -exec rm"),
    (r'\bfind\b.*-delete\b', "find -delete"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart morpheus-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart morpheus-gateway')"),
]
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("MORPHEUS_YOLO_MODE"):
        return {"approved": True, "message": None}
    ...
    session_key = os.getenv("MORPHEUS_SESSION_KEY", "default")
    if is_approved(session_key, pattern_key):
        return {"approved": True, "message": None}

    is_cli = os.getenv("MORPHEUS_INTERACTIVE")
    is_gateway = os.getenv("MORPHEUS_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("MORPHEUS_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("MORPHEUS_YOLO_MODE") or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("MORPHEUS_INTERACTIVE")
    is_gateway = os.getenv("MORPHEUS_GATEWAY_SESSION")
    is_ask = os.getenv("MORPHEUS_EXEC_ASK")
