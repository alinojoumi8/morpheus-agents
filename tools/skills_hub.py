#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Morpheus Skills Hub.
...
Used by morpheus_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            morpheus_meta = metadata.get("morpheus", {})
            if isinstance(morpheus_meta, dict):
                tags = morpheus_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                morpheus_meta = meta_block.get("morpheus", {})
                if isinstance(morpheus_meta, dict):
                    tags = morpheus_meta.get("tags", [])
