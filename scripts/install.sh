...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.morpheus/morpheus-agent)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/morpheus-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_node() {
    ...
    log_info "Extracting to ~/.morpheus/node/..."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Morpheus Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Morpheus behaves unexpectedly."
                    else
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Morpheus Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
copy_config_templates() {
    ...
    if [ ! -f "$HERMES_HOME/.env" ]; then
        ...
        log_info "~/.morpheus/.env already exists, keeping it"
    fi
    ...
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        ...
        log_info "~/.morpheus/config.yaml already exists, keeping it"
    fi
...
Edit this to customize how Morpheus communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.morpheus/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.morpheus/"

    # Seed bundled skills into ~/.morpheus/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.morpheus/skills/ ..."
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — Morpheus itself does not require root access."
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        python -m morpheus_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Morpheus to send/receive messages."
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.morpheus/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.morpheus/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.morpheus/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.morpheus/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.morpheus/morpheus-agent/"
    echo ""
