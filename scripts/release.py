...
VERSION_FILE = REPO_ROOT / "morpheus_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "morpheus@nousresearch.com": "NousResearch",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/morpheus-agent",
    ...
    lines.append(f"# Morpheus Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Morpheus Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Morpheus Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Morpheus Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        git("tag", "-a", tag_name, "-m",
            f"Morpheus Agent v{new_version} ({calver_date})\n\nWeekly release")
        ...
        result = subprocess.run(
            ["gh", "release", "create", tag_name,
             "--title", f"Morpheus Agent v{new_version} ({calver_date})",
        ...
        if result.returncode == 0:
            ...
            print(f"    gh release create {tag_name} --title 'Morpheus Agent v{new_version} ({calver_date})'")
