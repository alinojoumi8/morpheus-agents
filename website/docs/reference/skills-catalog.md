...
description: "Catalog of bundled skills that ship with Morpheus Agent"
---

# Bundled Skills Catalog

Morpheus ships with a large built-in skill library copied into `~/.morpheus/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `morpheus-agent-spawning` | Spawn additional Morpheus Agent instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate morpheus process. | `autonomous-ai-agents/morpheus-agent` |
...
| `morpheus-agent-setup` | Help users configure Morpheus Agent — CLI usage, setup wizard, model/provider selection, tools, skills, voice/STT/TTS, gateway, and troubleshooting. | `dogfood/morpheus-agent-setup` |
...
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Morpheus Agent tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| `morpheus-atropos-environments` | Build, test, and debug Morpheus Agent RL environments for Atropos training. Covers the MorpheusAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or f… | `mlops/training/morpheus-atropos-environments` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `google-workspace` | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Morpheus venv. | `productivity/google-workspace` |
...
| `plan` | Plan mode for Morpheus — inspect context, write a markdown plan into `.morpheus/plans/` in the active workspace/backend working directory, and do not execute the work. | `software-development/plan` |
