...
Morpheus Agent includes a full browser automation toolset with multiple backend options:
...
Instead of a cloud provider, you can attach Morpheus browser tools to your own running Chrome instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.
...
If Chrome isn't already running with remote debugging, Morpheus will attempt to auto-launch it with `--remote-debugging-port=9222`.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Morpheus can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
Screenshots are stored in `~/.morpheus/browser_screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.morpheus/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Morpheus automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
