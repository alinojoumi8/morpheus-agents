...
sidebar_position: 9
title: "Git Worktrees"
description: "Run multiple Morpheus agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Morpheus Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Morpheus so each session has a clean, isolated working directory.

## Why Use Worktrees with Morpheus?
...
- Morpheus checkpoint data under `~/.morpheus/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Morpheus experiment**
...
- Use **git worktrees** to give each Morpheus session its own clean checkout.
