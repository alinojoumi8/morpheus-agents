...
sidebar_position: 10
title: "DingTalk"
description: "Set up Morpheus Agent as a DingTalk chatbot"
---

# DingTalk Setup

Morpheus Agent integrates with DingTalk (钉钉) as a chatbot, letting you chat with your AI assistant through direct messages or group chats. The bot connects via DingTalk's Stream Mode — a long-lived WebSocket connection that requires no public URL or webhook server — and replies using markdown-formatted messages through DingTalk's session webhook API.

Before setup, here's the part most people want to know: how Morpheus behaves once it's in your DingTalk workspace.

## How Morpheus Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | Morpheus responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | Morpheus responds when you `@mention` it. Without a mention, Morpheus ignores the message. |
| **Shared groups with multiple users** | By default, Morpheus isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
Morpheus Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
**Fix**: Verify `DINGTALK_CLIENT_ID` and `DINGTALK_CLIENT_SECRET` are set correctly in `~/.morpheus/.env`. The Client ID is your AppKey, and the Client Secret is your AppSecret from the DingTalk Developer Console.
