...
description: "How to add a new inference provider to Morpheus Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Morpheus can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:
...
1. `morpheus_cli/auth.py` decides how credentials are found.
2. `morpheus_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `morpheus_cli/models.py`, `morpheus_cli/main.py`, and `morpheus_cli/setup.py` make the provider show up in the CLI.
...
1. `morpheus_cli/auth.py`
2. `morpheus_cli/models.py`
3. `morpheus_cli/runtime_provider.py`
4. `morpheus_cli/main.py`
5. `morpheus_cli/setup.py`
...
- `PROVIDER_REGISTRY` in `morpheus_cli/auth.py`
- `_PROVIDER_LABELS` in `morpheus_cli/models.py`
- `_PROVIDER_ALIASES` in both `morpheus_cli/auth.py` and `morpheus_cli/models.py`
- CLI `--provider` choices in `morpheus_cli/main.py`
...
## Step 2: Add auth metadata in `morpheus_cli/auth.py`
...
- What env vars should Morpheus check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `morpheus_cli/models.py`
...
## Step 4: Resolve runtime data in `morpheus_cli/runtime_provider.py`
...
Be careful with API-key precedence. Morpheus already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `morpheus_cli/main.py` and `morpheus_cli/setup.py`

A provider is not discoverable until it shows up in the interactive flows.

Update:

### `morpheus_cli/main.py`
...
When you add a native provider, double-check that Morpheus is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m morpheus_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m morpheus_cli.main model
...
- [ ] `ProviderConfig` added in `morpheus_cli/auth.py`
- [ ] aliases added in `morpheus_cli/auth.py` and `morpheus_cli/models.py`
- [ ] model catalog added in `morpheus_cli/models.py`
- [ ] runtime branch added in `morpheus_cli/runtime_provider.py`
- [ ] CLI wiring added in `morpheus_cli/main.py`
- [ ] setup wiring added in `morpheus_cli/setup.py`
