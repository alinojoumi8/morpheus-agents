...
description: "How Morpheus stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---

# Cron Internals

Morpheus cron support is implemented primarily in:

- `cron/jobs.py`
- `cron/scheduler.py`
- `tools/cronjob_tools.py`
- `gateway/run.py`
- `morpheus_cli/cron.py`
...
Cron jobs are stored in Morpheus-managed local state (`~/.morpheus/cron/jobs.json`) with atomic write semantics.
...
A cron job may attach multiple skills. At runtime, Morpheus loads those skills in order and then appends the job prompt as the task instruction.
...
Morpheus uses lock-based protections so overlapping scheduler ticks do not execute the same due-job batch twice.
