...
Morpheus currently supports three API execution modes:
...
Morpheus wraps API requests so they can be interrupted from the CLI or gateway.
...
Morpheus uses two execution strategies:
...
Morpheus tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, Morpheus may:
