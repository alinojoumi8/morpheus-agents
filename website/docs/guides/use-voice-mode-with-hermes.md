---
sidebar_position: 7
title: "Use Voice Mode with Morpheus"
description: "A practical guide to setting up and using Morpheus voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Morpheus
...
- you want Morpheus sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Morpheus.
...
## Step 1: make sure normal Morpheus works first
...
```bash
pip install "morpheus-agent[tts-premium]"
```
...
```bash
pip install "morpheus-agent[all]"
```
...
Morpheus supports both local and cloud speech stacks.
...
4. Morpheus transcribes and responds
...
- asking Morpheus to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Morpheus loop.

## Tuning CLI behavior

### Silence threshold

If Morpheus starts/stops too aggressively, tune:
...
Morpheus stays a normal chat bot, but can speak replies.
...
- you want Morpheus to function like a portable research or ops assistant
...
Morpheus joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Morpheus detects speech boundaries
- transcripts are posted in the associated text channel
- Morpheus responds in text and audio
...
1. get text Morpheus working
2. install `morpheus-agent[voice]`
