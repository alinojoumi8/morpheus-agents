...
This tutorial walks you through setting up a Telegram bot powered by Morpheus Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Morpheus Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/learning-path) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.morpheus/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Morpheus itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Morpheus Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.morpheus/.env`:
...
```
[Gateway] Starting Morpheus Gateway...
...
**Option 2:** Set it manually in `~/.morpheus/.env`:
...
Control how much detail the bot shows when using tools. In `~/.morpheus/config.yaml`:
...
| Session history | `~/.morpheus/sessions/` |
