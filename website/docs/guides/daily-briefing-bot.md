...
2. **Morpheus spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.morpheus/cron/output/` and you can read them anytime.
:::
...
Morpheus will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Morpheus Agent
```
...
Just tell Morpheus what you want:
...
Morpheus will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Morpheus to delegate each topic to a sub-agent:
...
Morpheus will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
