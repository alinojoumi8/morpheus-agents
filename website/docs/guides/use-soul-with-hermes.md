---
sidebar_position: 6
title: "Use SOUL.md with Morpheus"
description: "How to use SOUL.md to shape Morpheus Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Morpheus

`SOUL.md` is the **primary identity** for your Morpheus instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Morpheus to feel like the same assistant every time you talk to it — or if you want to replace the Morpheus persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Morpheus should be
- what Morpheus should avoid stylistically
- how Morpheus should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Morpheus is and how Morpheus speaks
...
Morpheus now uses only the global SOUL file for the current instance:

```text
~/.morpheus/SOUL.md
```

If you run Morpheus with a custom home directory, it becomes:
...
Morpheus automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Morpheus does not overwrite it
- if the file exists but is empty, Morpheus adds nothing from it to the prompt

## How Morpheus uses it

When Morpheus starts a session, it reads `SOUL.md` from `MORPHEUS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Morpheus feels.
...
Morpheus already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Morpheus should sound.

# Avoid
What Morpheus should not do.

# Defaults
How Morpheus should behave when ambiguity appears.
```
...
```bash
nano ~/.morpheus/SOUL.md
```

or

```bash
vim ~/.morpheus/SOUL.md
```

Then restart Morpheus or start a new session.
...
4. Talk to Morpheus for a while
...
### I edited SOUL.md but Morpheus still sounds the same

Check:
- you edited `~/.morpheus/SOUL.md` or `$MORPHEUS_HOME/SOUL.md`
...
### Morpheus is ignoring parts of my SOUL.md
